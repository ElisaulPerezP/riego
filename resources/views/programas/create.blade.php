        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <!-- Formulario para crear el programa de riego -->
                    <form action="{{ route('programa-riego.store') }}" method="POST">
                        @csrf

                        <!-- Veces por día -->
                        <div class="mb-6">
                            <label for="veces_por_dia" class="block text-sm font-medium text-gray-700">Veces por Día</label>
                            <input type="number" name="veces_por_dia" id="veces_por_dia" value="{{ old('veces_por_dia', 1) }}" min="1" class="mt-1 block w-full" required>
                        </div>
                            ...
                            @for ($i = 1; $i <= 14; $i++)
                                <div>
                                    ...
                                    <div class="bar-container" id="bar-container-{{ $i }}">
                                        <div class="bar bar-volumen" id="bar-volumen-{{ $i }}" style="height: {{ (old("volumen{$i}", 0) / 25) * 100 }}%;"></div>
                                        <div class="bar bar-fertilizante1" id="bar-fertilizante1-{{ $i }}" style="height: {{ (old("fertilizante1_{$i}", 0) / 10) * 100 }}%;"></div>
                                        <div class="bar bar-fertilizante2" id="bar-fertilizante2-{{ $i }}" style="height: {{ (old("fertilizante2_{$i}", 0) / 10) * 100 }}%;"></div>
                                    </div>
                                    ...
                                    <div class="mt-2">
                                        ...
                                        <input type="range" name="fertilizante1_{{ $i }}" id="fertilizante1_{{ $i }}" min="0" max="10" step="0.1" value="{{ old("fertilizante1_{$i}", 0) }}" class="w-full slider" data-bar-id="bar-fertilizante1-{{ $i }}" data-max="10">

                                        <!-- Fertilizante 2 -->
                                        <label for="fertilizante2_{{ $i }}" class="block text-sm font-medium text-gray-700">Fertilizante 2</label>
                                        <input type="range" name="fertilizante2_{{ $i }}" id="fertilizante2_{{ $i }}" min="0" max="10" step="0.1" value="{{ old("fertilizante2_{$i}", 0) }}" class="w-full slider" data-bar-id="bar-fertilizante2-{{ $i }}" data-max="10">
                                    </div>
                                </div>
                            @endfor
                        </div>

                        <!-- Botón para guardar el nuevo programa -->
                        <div class="mt-6">
